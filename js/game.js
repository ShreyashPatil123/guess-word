const Game = {
    ...
    resume() {
        ...
        UI.showScreen('game');
        UI.createGrid(this.config[this.state.difficulty].attempts, this.state.difficulty);
        UI.generateKeyboard();
        
        // Restore visuals
        UI.updateGrid(this.state.guesses, this.state.currentAttempt, this.state.currentGuess, this.config[this.state.difficulty].attempts, this.state.difficulty);
    ...
    handleInput(key) {
        try {
            ...
            if (key === 'BACKSPACE') {
                ...
                if (this.state.currentGuess.length == maxLen) { // Loose equality just in case
                    this.submitGuess();
                } else {
                    document.getElementById('grid-container').classList.add('animate-shake');
                    setTimeout(() => document.getElementById('grid-container').classList.remove('animate-shake'), 500);
    ...
    submitGuess() {
        try {
            ...
            guess.split('').forEach((char, i) => {
                ...
                else if (status === 'present' && current !== 'correct') this.state.letterStates[char] = 'present';
                else if (status === 'absent' && !current) this.state.letterStates[char] = 'absent';
            });

            // UI Updates
            UI.updateGrid(this.state.guesses, this.state.currentAttempt, '', this.config[this.state.difficulty].attempts, this.state.difficulty); 
            UI.updateKeyboard(this.state.letterStates);
            ...
            if (this.state.currentAttempt >= this.config[this.state.difficulty].attempts) {
    ...
    endGame(win, reason) {
        ...
        const score = this.calculateScore(win, this.state.timeLeft, this.state.currentAttempt + 1);
        ...
        // ==========================================
        if (typeof supabaseClient !== 'undefined' && Auth.currentUser && !this.state.gameFinalized) {
            ...
            supabaseClient.rpc('update_game_stats', {
        ...
        // Show Result
        setTimeout(() => {
            const m = Math.floor((this.config[this.state.difficulty].time - this.state.timeLeft) / 60);
            const s = (this.config[this.state.difficulty].time - this.state.timeLeft) % 60;
